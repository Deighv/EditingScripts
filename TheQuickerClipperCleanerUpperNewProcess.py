...
import datetime
import math
from os import walk, system, remove, startfile
import ffmpeg
...
while index <= NumberOfRuns-1: 
    ...
    toThis = int(fromThis+runInBatchesOf) #The number file in the group we wannt to end at
    ourFiles = []
    for x in range(fromThis, toThis): #don't use this it's silly.. unless you have a really good reason
        newName = "trimmed_and_clean_" + filenamesToStart[x]
        arguments = "-i " + filenamesToStart[x] + " -ss 00:00:1 -t 00:00:18 -map 0:v:0 -map 0:a:1 -map -0:a:2 -map -0:a:3 -map -0:a:4 -map -0:a:5 -map -0:a:6 -y "+ newName
        startfile("ffmpeg.exe","", arguments)
